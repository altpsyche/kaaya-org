...
| 21 | The build is blocked on content only the Kaaya team can supply | **Proxy values stand in, declared by a `KAAYA-PROXY(<ticket>): <what is owed>` comment beside the value, and `scripts/gate/proxy.mjs` counts them.** The Wix placeholder date and address are exempt from this and are refused outright. | Local development stalled behind five external blockers — mailboxes, event dates, five description strings, an exhibition photograph — while every task behind them is otherwise buildable. The alternative to a proxy is not "no invented content", it is a half-built tree nobody can review. What made invention dangerous was that it was untracked, so this makes it tracked: a comment cannot render, it works in YAML, markdown frontmatter, `.astro` and `.ts` alike, and it lives beside the value rather than in a register that drifts the first time someone deletes a proxy without updating it. `npm run gate:proxy` lists them and exits 0, since a proxy is meant to keep the tree building; `--strict` exits non-zero and is T9.4's cutover gate, so production cannot be reached with one still in the build. **The Wix placeholder date and address get no proxy at all** — they read as real and are wrong in a way a visitor would act on, so the same script fails on them in any mode, checked over built HTML because every page duplicates its YAML as a hardcoded fallback. |
