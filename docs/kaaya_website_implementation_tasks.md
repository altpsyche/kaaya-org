...
- **Landed:** `SITE.description` is deleted with 0 references remaining, and `SEO.astro` defaults to `SECTION_DESCRIPTION[sectionFor(pathname)]` — six strings, one per host. A page passing its own description still wins, which is why 18 of 25 routes are unchanged.
- **The mechanism's real job is the fallback, and that is where it shows.** Two pages take a per-host default today: the homepage, which dropped its INTERIM duplicate so the apex string lives in one place, and `404.astro`, which passed no description at all and was therefore the page carrying the banned phrase onto `gallery.kaaya.org`. Every other route states its own. Decision 6 removes the class of problem — a global default rendering on any page that forgets — rather than rewriting copy that is already specific.
- **`rss.xml.ts` was the third consumer and the build caught it**, failing with `[RSS] Invalid or missing options` the moment `SITE.description` went. The channel description is the blog's, so it takes `SECTION_DESCRIPTION.happenings` rather than a site-wide string — consistent with T2.3, where the channel `<link>` stays the apex because that element names the site.
- **`sectionFor()` moved into `links.ts`.** The Header, `ContactBlock` and `SEO` each derived the section from the path with their own copy of the same six lines; a fourth copy was the point to stop. Derived from the path and never from an href, which T2.5 records breaking silently.
- **`404.astro` carries the apex description on all six hosts**, the same limitation as its contact address: one file, six hostnames. It is no longer a banned phrase, which is what the AC asks.
- **The five real strings are still owed** — `KAAYA-PROXY(T4.6)` in `descriptions.ts`. The interim copy is written from what the site already says about itself rather than invented from nothing; `gate:vocab` enforces the ban independently. Proxies 4 → 5.
- Build 25 routes, `gate:vocab` 0 hits across 10 files, `gate:links` 0 dead 0 misrouted across 682 hrefs.
- **Interim copy was already in place, and this task replaced it rather than discovering it.** Three strings carried the banned phrase, not the one the description above names: `SITE.description`, `home.yaml`'s `introHeading`, and the homepage `<title>` — which also fed `og:title` and `twitter:title`, so it appeared 4× in `dist/index.html`. `SITE.description` was the one that mattered most: it is the fallback for any page passing no description of its own, and `404.astro` is one of those, so the phrase reached `gallery.kaaya.org` through the shared 404. All three now carry interim art-forward copy marked INTERIM in code comments, and `npm run gate:vocab` is green. **The AC as written is therefore already satisfied** — this task's real remaining work is the five real strings and the per-section derivation in `SEO.astro`.
...
- [x] **T7.5 — Build `gallery/shop/index.astro` and `gallery/shop/[slug].astro`**
...
- **Landed as one commit, not two, because the halves are not separable.** A catalogue whose cards link to detail pages that do not exist is 5 dead links, and `gate:links` refuses it — measured, not predicted. Routes **25 → 31**: the catalogue plus one page per work.
- **Acceptance criteria, line by line.** *No code sums, totals or persists `price`* — a grep for `price` next to arithmetic, `reduce`, `total`, `sum` or storage returns only the two comments saying it does not happen. *No cart or checkout route exists anywhere* — 0 built routes match cart, checkout, basket or order. *The page states plainly that buying happens by enquiry* — the catalogue opens with "Every piece here is one of a kind, and there is no checkout", which decision 4 requires because the label promises a checkout every button will refuse. *`/shop/chromatic-metanoia` resolves* — it builds, and the routing suite now serves it byte-identically on `gallery.kaaya.org`. *The enquiry path verified end to end* — **not verified, and blocked on T4.3 step 2**: the form posts `Shop Enquiry — Chromatic metanoia` to the `gallery` inbox with the work slug and title as hidden fields, so the enquiry names the piece without asking anyone to retype it, but `gallery@kaaya.org` is not a verified Web3Forms destination yet.
- **Three things that were waiting on these routes landed with them.** The Header's `UNBUILT` set is gone, so `gallery.kaaya.org` finally renders §5's full row 2 — Shop · Artist — and the section smoke test asserts both. The two routing rows skipped since T10.2 are now real tests. And the featured cards on the homepage and the gallery home turn their titles into links to the work, which T3.2 and T7.7 both recorded as owed here.
- **Full resolution stays reachable, which closes T7.9's deferred AC line.** The detail page renders a derivative and carries a "View at full resolution" link to `/uploads/chromatic-metanoia.png` — the original T7.9 never rewrites or deletes.
- **Only categories with entries are offered.** The filter reads `All work · Artworks` today, since Handmade and Collectibles hold nothing (blockers table). An empty category is never advertised, the same rule T8.1 established for events.
- **Four baselines moved:** routes 25 → 31, vocab files 10 → 16, links 684 → 894 hrefs on 27 → 32 pages, content leaf strings 202 → 205.
...
- **All three skips are now closed.** T7.5 built `/shop` and `/shop/chromatic-metanoia`, T8.3 imported the events, and each row became a live test in the commit that made it possible. The suite is 40 passed, 0 skipped — every row of §6.2's table asserted. Originally: the 3 skipped rows named the ticket that builds them — `/shop` and `/shop/chromatic-metanoia` (T7.5), `events.kaaya.org/artistry-weekend` (T8.3). Declared and skipped rather than omitted, so the gap is counted in the run instead of being invisible.
