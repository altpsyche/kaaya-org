...
- **Measured before planning, and the task is bigger than its own description.** The front door is the worst page, not the shop: `hero.png` is **5.7 MB on its own**, and T3.2 added three artwork PNGs below it, so `kaaya.org` costs **13.3 MB** of imagery today. The five artworks are 2.2–3.0 MB each. `sharp@0.34.5` is already present as an Astro dependency, so no new runtime dependency is needed. Nothing in the tree emits a `srcset` — six files carry an `<img>`, and `Hero.astro` is not one of them: it paints a CSS `background-image`, which cannot carry a `srcset` at all and needs `image-set()` instead.
...
- **The property that makes it safe: a missing derivative is not a broken image.** The component emits a `srcset` only for derivatives that exist at build time, so a fresh CMS upload, or a checkout where `npm run images` has not run, renders exactly the plain `<img>` the page renders today. That is what keeps a generated, gitignored directory from becoming a build dependency the next session has to know about.
  ...
  2. **[ ] Add `scripts/images.mjs` and wire it** — a `sharp` pass emitting `public/uploads/derived/<name>-{480,960,1440}.webp`, plus a 1920 for hero-sized sources, skipping anything already current. Gitignored, run by `npm run images`, wired ahead of both `build` and `dev`. Measures: sources in, derivatives out, total derived bytes, wall time. Build and all three gates unchanged, because no page reads a derivative yet.
  3. **[ ] Add `ResponsiveImage.astro` and adopt it on the three pages that show artwork** — the homepage highlights (T3.2), the gallery home's Featured Art (T7.7) and the artist detail grid (T7.6). Measures: the bytes a browser actually fetches at list size on `dist/index.html` and `dist/gallery/index.html`, before and after.
  4. **[ ] Convert `Hero.astro` to `image-set()`** — a `background-image` takes no `srcset`, so the hero declares the webp derivative with the original as its fallback, and a browser too old for `image-set()` gets exactly today's behaviour. Measures: homepage hero bytes, 5.7 MB → after.
